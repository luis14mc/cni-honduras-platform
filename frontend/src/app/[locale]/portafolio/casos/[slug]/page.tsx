...
const copy = {
  es: {
    ...
    metrics: [
      { icon: "groups", number: "+1,000", label: "Empleos Directos Generados", text: "Impacto socioeconómico continuo durante los doce meses del año en comunidades rurales.", border: "border-[#252A58]" },
      { icon: "public", number: "6+ Países", label: "Mercados de Exportación", text: "Presencia en USA, Reino Unido, Francia, Bélgica, Italia y Países Bajos con altos estándares.", border: "border-[#0E7A7C]" },
      { icon: "flight_takeoff", number: "Palmerola", label: "Hub Logístico Estratégico", text: "Garantía de frescura y trazabilidad absoluta desde el corazón de Centroamérica.", border: "border-[#35A963]" },
    ],
  ...
  en: {
    ...
    metrics: [
      { icon: "groups", number: "+1,000", label: "Direct Jobs Generated", text: "Continuous socio-economic impact during the twelve months of the year in rural communities.", border: "border-[#252A58]" },
      { icon: "public", number: "6+ Countries", label: "Export Markets", text: "Presence in USA, United Kingdom, France, Belgium, Italy and Netherlands with high standards.", border: "border-[#0E7A7C]" },
      { icon: "flight_takeoff", number: "Palmerola", label: "Strategic Logistics Hub", text: "Guarantee of freshness and absolute traceability from the heart of Central America.", border: "border-[#35A963]" },
    ],
    ...
    <div className="-mt-28 flex flex-1 flex-col bg-[#f8f9ff]">
      <header className="hero-gradient relative overflow-hidden pb-24 pt-48 md:pt-56">
        ...
        <div className="container relative z-10 mx-auto px-6 md:px-12">
          <Link href={L("/portafolio/casos")} className="mb-6 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#8DC046] hover:text-white">
            <MaterialIcon name="arrow_back" className="text-sm" />
            {c.backToList}
          </Link>
          <div className="max-w-4xl">
            <span className="mb-6 inline-block rounded-sm bg-[#0E7A7C] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-[#8DC046]">{c.storyLabel}: {title}</span>
            <h1 className="mb-8 text-4xl font-extrabold leading-tight tracking-tight text-white md:text-6xl">
              {title}: {subtitle}
            </h1>
            <div className="border-l-4 border-[#8DC046] py-2 pl-6">
              <p className="mb-4 text-xl font-light italic leading-relaxed text-white/90 md:text-2xl">{c.quote}</p>
              <cite className="font-bold not-italic tracking-wide text-[#8DC046]">{c.quoteAuthor}</cite>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="container mx-auto grid grid-cols-1 gap-8 px-6 md:grid-cols-3 md:px-12">
          {c.metrics.map((m) => (
            <div key={m.label} className={`rounded-xl border-t-4 bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 ${m.border}`}>
              <MaterialIcon name={m.icon} className="mb-4 block text-4xl text-[#0E7A7C]" />
              <h3 className="mb-2 text-4xl font-bold text-[#252A58]">{m.number}</h3>
              <p className="font-medium text-[#0E7A7C]">{m.label}</p>
              <p className="mt-4 text-sm leading-relaxed text-[#0E7A7C]/70">{m.text}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="flex flex-col items-start gap-16 lg:flex-row">
            <div className="space-y-8 lg:w-1/2">
              <div className="relative">
                <h2 className="text-3xl font-bold tracking-tight text-[#252A58]">{c.leftTitle}</h2>
                <span className="absolute -bottom-1 left-0 h-1 w-10 bg-[#8DC046]" />
              </div>
              <p className="text-lg leading-relaxed text-[#0E7A7C]">{c.leftText}</p>
            ...
            <div className="space-y-8 lg:w-1/2">
              <div className="relative">
                <h2 className="text-3xl font-bold tracking-tight text-[#252A58]">{c.rightTitle}</h2>
                <span className="absolute -bottom-1 left-0 h-1 w-10 bg-[#8DC046]" />
              </div>
              <p className="text-lg leading-relaxed text-[#0E7A7C]">{c.rightText}</p>
      ...
      <section className="relative overflow-hidden bg-[#252A58] py-24 text-white">
        ...
        <div className="container relative z-10 mx-auto px-6 md:px-12">
          <div className="max-w-3xl">
            ...
            <p className="mb-10 text-xl leading-relaxed text-[#b6c2d3]">{c.strategicText}</p>
            <div className="flex items-center gap-4">
              <div className="h-px flex-1 bg-[#2d476f]/30" />
              <MaterialIcon name="verified_user" filled className="text-[#8DC046]" />
              <div className="h-px flex-1 bg-[#2d476f]/30" />
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#f8f9ff] py-24">
        <div className="container mx-auto px-6 md:px-12">
          <div className="mb-12 flex items-end justify-between">
            <div>
              <h2 className="text-3xl font-extrabold tracking-tight text-[#252A58]">{c.relatedTitle}</h2>
              <p className="mt-2 text-[#0E7A7C]">{c.relatedSub}</p>
            </div>
            <Link href={L("/portafolio/casos")} className="hidden font-bold text-[#0E7A7C] underline-offset-4 hover:underline md:block">
              {c.seeAll}
            </Link>
          </div>
          <div className="grid grid-cols-1 gap-12 md:grid-cols-2">
            {c.related.map((r) => (
              <Link key={r.title} href={L(`/portafolio/casos/${r.title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`)} className="group cursor-pointer">
                ...
                <h3 className="mb-2 text-2xl font-bold text-[#252A58] transition-colors group-hover:text-[#0E7A7C]">{r.title}</h3>
                <p className="mb-4 line-clamp-2 text-[#0E7A7C]">{r.desc}</p>
                <span className="inline-flex items-center text-sm font-bold uppercase tracking-wide text-[#252A58]">
      ...
      <section className="bg-[#d3e4fe] py-20">
        <div className="container mx-auto px-6 text-center md:px-12">
          <div className="mx-auto max-w-2xl rounded-2xl bg-white p-12 shadow-xl shadow-[#252A58]/5">
            ...
            <Link
              href={L("/contacto")}
              className="inline-block rounded-md bg-[#252A58] px-10 py-4 text-lg font-bold text-white shadow-lg shadow-[#252A58]/20 transition-colors hover:bg-[#24436B] active:scale-95"
            >
              {c.ctaButton}
            </Link>
          </div>
