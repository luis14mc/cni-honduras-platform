...
const copy = {
  ...
  en: {
    ...
    cards: [
      { n: "01", title: "National Ecosystem Intelligence", items: ["Demographic Indicators", "Economic Indicators", "Social Indicators"], border: "border-[#252A58]" },
      { n: "02", title: "Commercial and Tourism Intelligence", items: ["Foreign Trade", "Sectors and Companies", "Tourism Trends"], border: "border-[#0E7A7C]" },
      { n: "03", title: "Investment and Competitiveness", items: ["Foreign Investment (FDI)", "Treaties and Agreements", "Infrastructure and Logistics"], border: "border-[#35A963]" },
    ],
  ...
  return (
    <div className="-mt-28 flex flex-1 flex-col bg-[#f8f9ff]">
      <section className="relative w-full overflow-hidden px-8 py-24 md:py-32">
        <div className="absolute inset-0 z-0 bg-gradient-to-br from-[#252A58] via-[#24436B] to-[#0E7A7C] opacity-95" />
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-12 md:grid-cols-2">
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              <Link href={L("/recursos")} className="rounded-md bg-[#35A963] px-8 py-4 text-sm font-bold text-[#110a00] transition-all hover:scale-105 active:scale-95">
                {c.primaryCta}
              </Link>
              <Link
                href={L("/recursos")}
      ...
      <section className="mx-auto max-w-7xl px-8 py-24">
        <div className="mb-16">
          <span className="mb-4 block text-xs font-bold uppercase tracking-widest text-[#0E7A7C]">{c.sectionEyebrow}</span>
          <h2 className="text-4xl font-extrabold tracking-tighter text-[#252A58]">{c.sectionTitle}</h2>
        </div>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {c.cards.map((card) => (
            <div key={card.n} className={`group rounded-xl border-t-4 bg-white p-10 shadow-[0_20px_40px_rgba(0,10,30,0.04)] ${card.border}`}>
              <div className="mb-8 text-6xl font-black text-[#252A58]/5 transition-colors duration-500 group-hover:text-[#35A963]">{card.n}</div>
              <h3 className="mb-6 text-xl font-bold text-[#252A58]">{card.title}</h3>
              <ul className="space-y-4">
                {card.items.map((it) => (
                  <li key={it} className="flex items-center gap-3 font-medium text-[#0E7A7C]">
                    <span className="h-1.5 w-1.5 rounded-full bg-[#0E7A7C]" />
                    {it}
                  </li>
                ))}
              </ul>
      ...
      <section className="bg-[#eff4ff] px-8 py-24">
        <div className="mx-auto max-w-7xl">
          <div className="mb-16 text-center">
            <h2 className="mb-4 text-3xl font-extrabold tracking-tighter text-[#252A58] md:text-4xl">{c.keyTitle}</h2>
            <p className="mx-auto max-w-2xl text-[#0E7A7C]">{c.keySub}</p>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {c.keyItems.map((it) => (
              <div key={it.title} className="group rounded-lg bg-white p-8 shadow-[0_20px_40px_rgba(0,10,30,0.04)] transition-all hover:-translate-y-1">
                <MaterialIcon name={it.icon} className="mb-6 block text-4xl text-[#0E7A7C] transition-transform group-hover:scale-110" />
                <h4 className="mb-2 text-sm font-bold text-[#252A58]">{it.title}</h4>
                <p className="text-xs leading-relaxed text-[#0E7A7C]">{it.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-8 py-24">
        <div className="relative mx-auto max-w-4xl overflow-hidden rounded-2xl bg-[#252A58] p-12 text-center">
          <div className="absolute -mr-32 -mt-32 right-0 top-0 h-64 w-64 rounded-full bg-[#0E7A7C]/10 blur-3xl" />
          <div className="absolute -mb-32 -ml-32 bottom-0 left-0 h-64 w-64 rounded-full bg-[#35A963]/5 blur-3xl" />
          ...
          <a href="#" className="mx-auto flex w-fit items-center gap-3 rounded-md bg-white px-10 py-4 text-sm font-extrabold text-[#252A58] transition-all hover:bg-[#35A963] active:scale-95">
