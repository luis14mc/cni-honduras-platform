...
const copy = {
  es: {
    ...
    sectors: ["Agroindustria", "Manufactura", "Turismo", "Energía", "Infraestructura", "BPO / Call Centers", "Otro"],
  ...
  en: {
    ...
    sectors: ["Agroindustry", "Manufacturing", "Tourism", "Energy", "Infrastructure", "BPO / Call Centers", "Other"],
    sizes: ["< 1M", "1M - 5M", "5M - 25M", "25M - 100M", "> 100M"],
...
export default async function AsesoriaPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="flex flex-1 flex-col bg-[#f8f9ff]">
      ...
      <section className="relative -mt-28 flex min-h-[480px] items-center overflow-hidden bg-[#252A58] pt-28">
        <Image
      ...
      <section className="mx-auto w-full max-w-screen-2xl px-6 py-20 md:px-10">
        <h2 className="text-3xl font-extrabold tracking-tight text-[#252A58] md:text-4xl">{c.benefitsTitle}</h2>
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {c.benefits.map((b) => (
            <article key={b.title} className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-black/5">
              <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#252A58] text-[#35A963]">
                <MaterialIcon name={b.icon} className="!text-2xl" />
              </span>
              <h3 className="mt-5 text-lg font-bold text-[#252A58]">{b.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-[#0E7A7C]">{b.text}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="bg-white py-20" id="formulario">
        <div className="mx-auto grid w-full max-w-screen-2xl gap-12 px-6 md:px-10 lg:grid-cols-[1fr_1.4fr]">
          <aside className="space-y-6">
            <h2 className="text-3xl font-extrabold tracking-tight text-[#252A58] md:text-4xl">{c.formTitle}</h2>
            <p className="text-base leading-relaxed text-[#0E7A7C]">{c.formSubtitle}</p>
            <p className="rounded-lg bg-[#f8f9ff] p-5 text-xs leading-relaxed text-[#0E7A7C] ring-1 ring-black/5">
              <MaterialIcon name="lock" className="!text-base align-middle text-[#0E7A7C]" /> {c.privacy}
            </p>
          </aside>
          <form
            className="grid grid-cols-1 gap-5 rounded-2xl bg-[#f8f9ff] p-8 ring-1 ring-black/5 md:grid-cols-2"
            aria-label={c.formTitle}
          >
            <label className="flex flex-col gap-2 md:col-span-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelName}</span>
              <input type="text" required className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40" />
            </label>
            <label className="flex flex-col gap-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelCompany}</span>
              <input type="text" required className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40" />
            </label>
            <label className="flex flex-col gap-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelCountry}</span>
              <input type="text" required className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40" />
            </label>
            <label className="flex flex-col gap-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelEmail}</span>
              <input type="email" required className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40" />
            </label>
            <label className="flex flex-col gap-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelPhone}</span>
              <input type="tel" className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40" />
            </label>
            <label className="flex flex-col gap-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelSector}</span>
              <select required className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40">
                <option value="">—</option>
                {c.sectors.map((s) => (
            ...
            <label className="flex flex-col gap-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelSize}</span>
              <select className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40">
                <option value="">—</option>
                {c.sizes.map((s) => (
                  <option key={s}>{s}</option>
                ))}
            ...
            <label className="flex flex-col gap-2 md:col-span-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#252A58]">{c.labelMessage}</span>
              <textarea rows={5} className="rounded-md border border-black/10 bg-white px-4 py-3 text-sm focus:border-[#252A58] focus:outline-none focus:ring-2 focus:ring-[#35A963]/40" />
            </label>
            <button
              type="submit"
              className="mt-3 inline-flex items-center justify-center gap-2 rounded-full bg-[#252A58] px-8 py-4 text-xs font-bold uppercase tracking-[0.2em] text-[#35A963] transition hover:bg-[#0E7A7C] md:col-span-2"
            >
              {c.submit}
